...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for idx, locus in enumerate(loci, start=1):
            ...
            sorted_group["is_holdout"] = sorted_group["protein_sequence"].str.upper().isin(holdout_seqs)
            sorted_group["gbk_path"] = str(gbk)
            sorted_group["window_start"] = window_start
            sorted_group["window_end"] = window_end
            sorted_group["peptidase_accession"] = locus["accession"]
            all_df_rows.append(sorted_group)
